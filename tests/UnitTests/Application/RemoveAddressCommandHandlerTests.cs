...
public sealed class RemoveAddressCommandHandlerTests
{
    private static readonly DateTimeOffset Now = new(2026, 1, 1, 0, 0, 0, TimeSpan.Zero);

    [Fact]
    public async Task Handle_ExistingAddress_RemovesItAndPublishesUserProfileUpdated()
    {
        ...
        var handler = new RemoveAddressCommandHandler(dbContext, new FixedDateTimeProvider(Now));
        var result = await handler.Handle(new RemoveAddressCommand("user-1", address.AddressId, "user-1"), CancellationToken.None);
    ...
    public async Task Handle_UnknownAddress_ReturnsNotFound()
    {
        ...
        var handler = new RemoveAddressCommandHandler(dbContext, new FixedDateTimeProvider(Now));
        var result = await handler.Handle(new RemoveAddressCommand("user-1", Guid.NewGuid(), "user-1"), CancellationToken.None);
